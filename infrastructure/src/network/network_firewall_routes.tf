...
################################################################################
# Public Subnet Routes Through Firewall
################################################################################
...
resource "aws_route_table" "public_firewall" {
  ...
  tags = merge(local.common_tags, {
    Name = "${local.resource_prefix}-public-firewall-rt-${count.index + 1}"
  })
}

resource "aws_route" "public_firewall_internet" {
  count = var.enable_network_firewall ? length(local.azs) : 0
...
resource "aws_route" "public_firewall_private" {
  count = var.enable_network_firewall ? length(local.azs) : 0
